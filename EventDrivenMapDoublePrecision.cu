...
#include <curand.h>
#include <assert.h>
#include "EventDrivenMapDoublePrecision.hpp"
...
EventDrivenMap::EventDrivenMap(const arma::vec* pParameters, unsigned int noReal)
{
  ...
  *mpHost_p = arma::conv_to<arma::fvec>::from(*pParameters);
  ...
  CUDA_CALL( cudaMalloc( &mpDev_p, mpHost_p->n_elem*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_beta, mNoReal*mNoThreads*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_v, mNoReal*mNoThreads*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_s, mNoReal*mNoThreads*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_w, mNoThreads*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_U, (noSpikes+1)*sizeof(double) ));
  CUDA_CALL( cudaMalloc( &mpDev_lastSpikeInd,
        mNoReal*noSpikes*sizeof(unsigned short) ));
  CUDA_CALL( cudaMalloc( &mpDev_lastSpikeTime, mNoReal*noSpikes*sizeof(double)
        ));
  CUDA_CALL( cudaMalloc( &mpDev_crossedSpikeInd,
        mNoReal*noSpikes*sizeof(unsigned short) ));
  CUDA_CALL( cudaMalloc( &mpDev_crossedSpikeTime,
        mNoReal*noSpikes*sizeof(double) ));
  ...
  CUDA_CALL( cudaMemcpy(mpDev_p,mpHost_p->begin(),(*mpHost_p).n_elem*sizeof(double),cudaMemcpyHostToDevice));
...
{
  double *w;
  w = (double*) malloc( mNoThreads*sizeof(double));
  for (int i=0;i<mNoThreads;++i)
  {
    double x = -L + (double)(2*L/mNoThreads)*i;
    w[i] = (a1*exp(-b1*abs(x))-a2*exp(-b2*abs(x)))*2*L/mNoThreads;
  }
  circshift(w,mNoThreads/2,mNoThreads);
  CUDA_CALL( cudaMemcpy(mpDev_w,w,mNoThreads*sizeof(double),cudaMemcpyHostToDevice));
                               ...
                               const arma::vec& ZTilde)
{
  ...
  // Copy data to GPU
  //cudaMemcpy( dev_w, w, mNoThreads*sizeof(double), cudaMemcpyHostToDevice );
...
void EventDrivenMap::SetTimeHorizon( const double T)
{
  assert(T>0);
  mFinalTime = T;
...
void EventDrivenMap::SetParameterStdDev( const double sigma)
{
  assert(sigma>=0);
  mParStdDev = sigma;
...
void EventDrivenMap::SetParameters( const unsigned int parId, const double parVal)
{
  assert(parId<=(*mpHost_p).n_elem);
  (*mpHost_p)[parId] = parVal;
  CUDA_CALL( cudaMemcpy(mpDev_p+parId,&parVal,sizeof(double),cudaMemcpyHostToDevice));
...
void EventDrivenMap::initialSpikeInd( const arma::vec& U)
{
  ...
  for (m=1;m<noSpikes;m++) {
    for (i=mpHost_lastSpikeInd[m-1];i>0;i--) {
      if (-L+(double)(2*i*L/mNoThreads)<-U[0]*U[m]) {
...
void EventDrivenMap::SaveInitialSpikeInd()
{
  ...
  {
    fprintf(fp,"%f\n",(double) mpHost_lastSpikeIndBig[i]);
  }
  fclose(fp);
...
{
  double* mpHost_Lift;
  mpHost_Lift = (double*) malloc( 2*mNoReal*mNoThreads*sizeof(double));
  CUDA_CALL( cudaMemcpy( mpHost_Lift,mpDev_v,mNoReal*mNoThreads*sizeof(double),cudaMemcpyDeviceToHost));
  CUDA_CALL( cudaMemcpy( mpHost_Lift+mNoReal*mNoThreads,mpDev_s,mNoReal*mNoThreads*sizeof(double),cudaMemcpyDeviceToHost));
...
void EventDrivenMap::SaveEvolve()
{
  ...
  double* mpHost_spikeTime;
  mpHost_spikeInd = (unsigned short*) malloc( noSpikes*mNoReal*sizeof(short));
  mpHost_spikeTime = (double*) malloc( noSpikes*mNoReal*sizeof(double));
  ...
  CUDA_CALL( cudaMemcpy( mpHost_spikeTime, mpDev_lastSpikeTime, noSpikes*mNoReal*sizeof(double), cudaMemcpyDeviceToHost));

  SaveData(noSpikes*mNoReal,mpHost_spikeTime,filename2);
  for (int i=0;i<noSpikes*mNoReal;i++)
  {
    mpHost_spikeTime[i] = (double)mpHost_spikeInd[i];
  }
  ...
  CUDA_CALL( cudaMemcpy( mpHost_spikeTime, mpDev_crossedSpikeTime, noSpikes*mNoReal*sizeof(double), cudaMemcpyDeviceToHost));
  ...
  {
    mpHost_spikeTime[i] = (double)mpHost_accept[i];
  }
...
void EventDrivenMap::SaveRestrict()
{
  ...
  double* mpHost_averages;
  mpHost_averages = (double*) malloc( noSpikes*mNoReal*sizeof(double));
  cudaMemcpy( mpHost_averages, mpDev_lastSpikeTime, noSpikes*mNoReal*sizeof(double), cudaMemcpyDeviceToHost);
...
void EventDrivenMap::SaveAveraged()
{
  ...
  double* mpHost_averages;
  mpHost_averages = (double*) malloc( noSpikes*sizeof(double));
  cudaMemcpy( mpHost_averages, mpDev_U, noSpikes*sizeof(double), cudaMemcpyDeviceToHost);
    ...
    const unsigned int noReal)
{
  ...
  if(k<blockDim.x*noReal){
    ...
    //Define x-array
    double x = L - (double)(2*L/blockDim.x)*threadIdx.x;
    double s = 0.0f;
    double c = U[0];
    double beta = par[0];
    double dummyV, dummyS = 0.0f;
...
__device__ double eventTime( double v0, double s0, double beta)
{
  ...
  double f, df, estimatedTime = 0.0f;
...
__global__ void EvolveKernel( double *v, double *s, const double *beta,
    ...
    double *global_lastSpikeTime, unsigned short *global_crossedSpikeInd,
    double *global_crossedSpikeTime, unsigned int *global_accept, unsigned int noReal)
{
...
__global__ void EvolveKernel( double *v, double *s, const double *beta,
    const double *w, const double finalTime, unsigned short *global_lastSpikeInd,
    double *global_lastSpikeTime, unsigned short *global_crossedSpikeInd,
    double *global_crossedSpikeTime, unsigned int noReal)
{
  __shared__ unsigned short local_lastSpikeInd[noSpikes];
  __shared__ unsigned short local_crossedSpikeInd[noSpikes];
  __shared__ double local_lastSpikeTime[noSpikes];
                                ...
                                const unsigned int noReal)
{
  ...
  {
    double t0 = global_lastSpikeTime[index];
    double t1 = global_crossedSpikeTime[index];
    double x0 = -L+2.0f*L/blockDim.x*global_lastSpikeInd[index];
    double x1 = -L+2.0f*L/blockDim.x*global_crossedSpikeInd[index];
...
__global__ void realisationReductionKernelBlocks( double *V,
                                                  const double *U,
                                                  const unsigned int noReal,
